...
class CustomTitlebar extends _get__("themebar_1").ThemeBar {
  ...
  setupMenubar() {
    ...
    _get__("electron_1").ipcRenderer.send('window-set-minimumSize', this.defaultOptions.minWidth, this.defaultOptions.minHeight);
  }
  setupTitle() {
    const onlyRendererMenuBar = this.defaultOptions.onlyShowMenuBar;
  ...
  setupContainer() {
    ...
    if (this.containerExisted) {
      (0, _get__("dom_1").removeClass)(this.container, _get__("consts_1").DISPOSED_CONTAINER_CLASS);
    } else {
  ...
  loadEvents() {
    ...
    _get__("electron_1").ipcRenderer.on('window-maximize', this._onIpcMax);
    _get__("electron_1").ipcRenderer.on('window-fullscreen', this._onIpcFull);
    _get__("electron_1").ipcRenderer.on('window-focus', this._onIpcFocus);
    this._wiredIpc = true;
    if (minimizable) {
    ...
    if (_get__("platform_1").isMacintosh) {
      this._domDisposables.push((0, _get__("dom_1").addDisposableListener)(this.controls.minimize, _get__("dom_1").EventType.CLICK, () => {
        _get__("electron_1").ipcRenderer.send('window-event', 'window-minimize');
      }));
    }
    if (maximizable) {
  ...
  updateMenu(menu) {
    ...
    this.menuBar = new (_get__("menubar_1").MenuBar)(this.menuBarContainer, _get__("consts_1").menuIcons, this.defaultOptions, {
      enableMnemonics: true
  ...
  updateStyles() {
    if (this.isInactive) (0, _get__("dom_1").addClass)(this.titlebar, 'inactive');else (0, _get__("dom_1").removeClass)(this.titlebar, 'inactive');
    // --- Normalize backgroundColor to Color instance if a hex string slipped in ---
    const normalize = c => typeof c === 'string' ? _get__("color_1").Color.fromHex(c) : c;
    const baseBg = normalize(this.defaultOptions.backgroundColor);
    const backgroundColor = this.isInactive && this.defaultOptions.unfocusEffect ? baseBg?.lighten(0.12) : baseBg;
    ...
    if (backgroundColor?.isLighter()) {
      ...
      foregroundColor = this.isInactive && this.defaultOptions.unfocusEffect ? _get__("consts_1").INACTIVE_FOREGROUND_DARK : _get__("consts_1").ACTIVE_FOREGROUND_DARK;
    } else {
      (0, _get__("dom_1").removeClass)(this.titlebar, 'light');
      foregroundColor = this.isInactive && this.defaultOptions.unfocusEffect ? _get__("consts_1").INACTIVE_FOREGROUND : _get__("consts_1").ACTIVE_FOREGROUND;
    }
    ...
    if (this.menuBar) {
      const mbBg = normalize(this.defaultOptions.menuBarBackgroundColor) || normalize(this.defaultOptions.backgroundColor)?.darken(0.12);
      const mbFg = mbBg?.isLighter() ? _get__("consts_1").INACTIVE_FOREGROUND_DARK : _get__("consts_1").INACTIVE_FOREGROUND;
      const itemBgOpt = normalize(this.defaultOptions.itemBackgroundColor);
      const selBg = itemBgOpt && mbBg && !itemBgOpt.equals(mbBg) ? itemBgOpt : _get__("consts_1").DEFAULT_ITEM_SELECTOR;
      const selFg = selBg?.equals(_get__("consts_1").DEFAULT_ITEM_SELECTOR) ? mbBg?.isLighter() ? _get__("consts_1").ACTIVE_FOREGROUND_DARK : _get__("consts_1").ACTIVE_FOREGROUND : selBg?.isLighter() ? _get__("consts_1").ACTIVE_FOREGROUND_DARK : _get__("consts_1").ACTIVE_FOREGROUND;
  ...
  updateTitleAlignment(side) {
    ...
    if (side === 'center') {
      ...
      if (menuPosition !== 'bottom') {
        this._domDisposables.push((0, _get__("dom_1").addDisposableListener)(window, 'resize', () => {
          if (this.canCenterTitle()) (0, _get__("dom_1").addClass)(this.title, 'cet-title-center');else (0, _get__("dom_1").removeClass)(this.title, 'cet-title-center');
        }));
  ...
  dispose() {
    ...
    if (this._wiredIpc) {
      if (this._onIpcMax) _get__("electron_1").ipcRenderer.off('window-maximize', this._onIpcMax);
      if (this._onIpcFull) _get__("electron_1").ipcRenderer.off('window-fullscreen', this._onIpcFull);
      if (this._onIpcFocus) _get__("electron_1").ipcRenderer.off('window-focus', this._onIpcFocus);
    ...
    (0, _get__("dom_1").addClass)(this.container, _get__("consts_1").DISPOSED_CONTAINER_CLASS);
  }
  get titlebarElement() {
